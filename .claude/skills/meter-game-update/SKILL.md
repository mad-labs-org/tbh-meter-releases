...
**But a GREEN static diff is NEVER ship-ready (bucket 4 — see the guide).** On **1.00.20** the recompile
**killed the ACTk `fakeValue` decoy** that fed live `HeroRuntime` level/exp: **zero offset/index movement
(static preflight FULLY GREEN), reseed correct** — yet `read_live_party` read `lvl=0` for every real
deployed hero → `sm=NOT found` → party/`xp-live`/`stats` all broke. The diff can't see an encoding change;
**only `validate_live` (step 6) caught it.** The fix was NEITHER a reseed NOR an offset edit: the Obscured
cipher is off-limits ([[invariants/obscured-data-offlimits]]), so live level/exp **degrade to the save**
while the party *identity* stays LIVE, and the dead `lvl>0` ghost filter became a **catalog-heroKey** check
([[invariants/party-live-resolution]]). Moral: **`validate_live` PASS on ALL metrics is the gate — a green diff is not.**
...
- Don't edit `config/offsets.py` "to be safe" — only when step 3 shows a real `✗`. On 1.00.10 nothing shifted. And the inverse trap: a GREEN diff + a LIVE FAIL on the party/xp/stats path is a **bucket-4 encoding change** (1.00.20's dead Obscured decoy), NOT an offset problem — do NOT go editing offsets to chase it; the fix is a read-strategy change (degrade the dead value to the save, keep the party *identity* live — see the guide's bucket 4 + [[party-live-resolution]]).
...
- Don't ship on a PARTIAL check (selftest + diff + "the gold works"). The obfuscated classes (party/xp via `HeroRuntime`, gold via `AggregateManager`) only validate LIVE — `validate_live.py` must PASS on ALL metrics. Validating only the field you touched is exactly how the party bug shipped (1.00.11). And a FULLY GREEN static diff is not ship-readiness either: on 1.00.20 every offset/index was unchanged yet party/`xp-live`/`stats` broke live (a build-wide-dead Obscured decoy) — only `validate_live` PASS on ALL metrics is the gate.
